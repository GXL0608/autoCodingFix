...
import { AutofixAuto } from "../../src/autofix/auto"
...
test("ask - auto replies in autofix mode", async () => {
  await using tmp = await tmpdir({ git: true })
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      const sessionID = SessionID.make("ses_auto")
        ...
        questions: [
          {
            ...
            header: "Path",
            options: [
              { label: "Keep API", description: "Preserve the existing API surface" },
              { label: "Drop API (Recommended)", description: "Reduce scope and risk" },
            ],
          },
          {
            ...
            header: "Order",
            options: [
              { label: "Fix UI first", description: "Start from the smallest visible change" },
              { label: "Refactor first", description: "Do the bigger internal change first" },
            ],
          },
        ],
      })

      expect(answers).toEqual([["Drop API (Recommended)"], ["Fix UI first"]])
      expect(await Question.list()).toEqual([])
      AutofixAuto.disable(sessionID)
    },
  })
})
