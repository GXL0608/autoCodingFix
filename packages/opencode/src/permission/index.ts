...
import { AutofixAuto } from "@/autofix/auto"
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const ask = Effect.fn("Permission.ask")(function* (input: z.infer<typeof AskInput>) {
        ...
        if (!needsAsk) return
        if (AutofixAuto.has(request.sessionID)) {
          log.info("auto approved", { permission: request.permission, patterns: request.patterns })
          if (request.permission === "doom_loop") {
            return yield* new CorrectedError({
              feedback: "Autofix auto mode stopped a repeated tool-call loop",
            })
          }
          return
        }
